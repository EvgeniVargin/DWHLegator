...
BEGIN
  ...
  IF (col_num IS NOT NULL) THEN
    LOOP
      ...
      rec.col_type := CASE rec_tab(col_num).col_type WHEN 1 THEN 'VARCHAR2' WHEN 96 THEN 'VARCHAR2' WHEN 2 THEN 'NUMBER' WHEN 112 THEN 'CLOB' WHEN 113 THEN 'BLOB' WHEN 12 THEN 'DATE' ELSE to_char(rec_tab(col_num).col_type) END;
...
FUNCTION ReplGetImpScript(inGroupID IN NUMBER,inMask IN VARCHAR2 DEFAULT '11111111',inOwner VARCHAR2 DEFAULT NULL) RETURN CLOB
  IS
  vOwner VARCHAR2(30) := CASE WHEN inOwner IS NULL THEN GetVarValue('vOwner') ELSE UPPER(inOwner) END;
  vOwnerOld VARCHAR2(30) := GetVarValue('vOwner');
  --
...
BEGIN
  dbms_output.put_line(vOwner||'|'||vOwnerOld);
  ...
  IF getImpAggr THEN
    ...
    FOR idx IN (
      ...
      'vAggrSQL := REGEXP_REPLACE(REGEXP_REPLACE(q''['||REPLACE(REPLACE(idx.aggr_sql,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'',''\.anltline_''||vOldGrpID||''#|\.ANLTLINE_''||vOldGrpID||''#'',''.anltline_''||vGrpID||''#''),''\.fct_''||vOldFctGrpID||''|\.FCT_''||vOldFctGrpID,''.fct_''||vFctGrpID);'||CHR(10)||
      'FOR dim IN ('||CHR(10)||
      '  WITH'||CHR(10)||
      '    t AS ('||CHR(10)||'SELECT /*+ materialize */ group_id,table_name,entity_id,entity_name FROM ('||CHR(10);
      ...
      vCou := 0;
      FOR idx IN (
        SELECT table_name,entity_id,entity_name
          FROM TABLE(pkg_etl_signs.StarGetFldList(TRUNC(SYSDATE,'DD'),inGroupID))
          WHERE entity_id IS NOT NULL
        GROUP BY table_name,entity_id,entity_name
      ) LOOP
        vBuff :=
        CHR(10)||CASE WHEN vCou > 0 THEN 'UNION ALL'||CHR(10) END||
        'SELECT '||inGroupID||' AS group_id'||
        ','''||idx.table_name||''' as table_name'||
        ','||idx.entity_id||' AS entity_id'||
        ','''||idx.entity_name||''' AS entity_name FROM dual';
        dbms_lob.writeappend(vAggrDML,LENGTH(vBuff),vBuff);
        vCou := vCou + 1;
      END LOOP;
  ...
  IF getImpSigns THEN
    ...
    FOR idx IN (
      WITH
       ...
       ,sgn_all AS (
          SELECT /*+ materialize */ DISTINCT sign_name
            FROM tb_sign_2_sign
          CONNECT BY PRIOR prev_sign_name = sign_name
          START WITH sign_name IN (SELECT sign_name FROM sgn)
        UNION
        SELECT sign_name FROM sgn
        )
      ...
      SELECT p.sign_name
              ...
              FROM tb_signs_pool p
                   LEFT JOIN tb_entity e
                     ON e.id = p.entity_id
              WHERE p.sign_name IN (SELECT sign_name FROM sgn_all)
        ORDER BY p.id
      ...
      'vSignSQL := q''['||REPLACE(REPLACE(idx.sign_sql,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'';'||CHR(10)||
      'vMassSQL := q''['||REPLACE(REPLACE(idx.mass_sql,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'';'||CHR(10)||
      'vExtPLSQL := q''['||REPLACE(REPLACE(idx.ext_plsql,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'';'||CHR(10)||
      'vCondition := q''['||REPLACE(REPLACE(idx.condition,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'';'||CHR(10)||
  ...
  IF getImpAnlt THEN
    ...
    FOR idx IN (
      ...
      vBuff :=
      'vAnltSQL := q''['||REPLACE(REPLACE(idx.anlt_sql,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'';'||CHR(10)||
      'vEntID := GetEntIdByName(q''['||idx.entity_name||']'');'||CHR(10)||
      ...
      FOR spec IN (
        ...
        vBuff :=
        'vAnltSpecCond := q''['||REPLACE(REPLACE(spec.condition,UPPER(vOwnerOld)||'.',UPPER(vOwner)||'.'),LOWER(vOwnerOld)||'.',LOWER(vOwner)||'.')||']'';'||CHR(10)||
...
FUNCTION RegistryCreateObjDDL(inTableName IN VARCHAR2
  ...
  IS
    ...
    vCLOB CLOB;
    vSeq BOOLEAN := SUBSTR(inMask,1,1) = '1';
    vTbl BOOLEAN := SUBSTR(inMask,2,1) = '1';
    vTrg BOOLEAN := SUBSTR(inMask,3,1) = '1';
    vPK BOOLEAN := SUBSTR(inMask,4,1) = '1';
    vFK BOOLEAN := SUBSTR(inMask,5,1) = '1';
    --
    vBuff VARCHAR2(32700);
    vFirst INTEGER := 0;
BEGIN
  ...
  IF vSeq THEN
    ...
    FOR idx IN (
      SELECT s.seq_name
            ...
            ,s.cycle_flag
            ,s.cache_size
        FROM tb_seq_registry s
             INNER JOIN tb_tfield_registry f
               ON f.sequence_name = SUBSTR(s.seq_name,INSTR(seq_name,'.') + 1)
             INNER JOIN tb_tables_registry t
               ON t.id = f.table_id
                  AND t.table_name = UPPER(inTableName)
    ) LOOP
      vBuff := vBuff||CHR(10)||
               '  EXECUTE IMMEDIATE ''CREATE SEQUENCE '||inOwner||'.'||SUBSTR(idx.seq_name,INSTR(idx.seq_name,'.') + 1)||' MINVALUE '||idx.min_value||' MAXVALUE '||idx.max_value||
               '    INCREMENT BY '||idx.increment_by||' START WITH 1 '||CASE WHEN idx.cycle_flag = 'N' THEN 'NOCYCLE' ELSE 'CYCLE' END||
               ' '||CASE WHEN idx.cache_size = 0 THEN 'NOCACHE' ELSE 'CACHE '||idx.cache_size END||''';';
    END LOOP;
          ...
          SELECT f.ord,f.field_name,f.field_type,f.sequence_name,o.opt_name,o.opt_val
            FROM tb_tables_registry t
                 ...
                 LEFT JOIN tb_tfoption_registry o
                   ON o.field_id = f.id
            WHERE t.table_name = UPPER(inTableName)
        ) PIVOT (MAX(opt_val) FOR opt_name IN ('DATA_LENGTH' AS data_length,'DEFAULT_VALUE' AS default_value,'NOT_NULL' AS not_null))
      ORDER BY ord
    ) LOOP
      vBuff := vBuff||CHR(10)||CASE WHEN vFirst > 0 THEN ',' ELSE ' ' END||idx.field_name||' '||idx.field_type||CASE WHEN idx.data_length IS NOT NULL AND NOT(idx.field_type IN ('CLOB','BLOB')) THEN '('||idx.data_length||')' END||CASE WHEN idx.default_value IS NOT NULL THEN ' DEFAULT '||idx.default_value END||CASE WHEN idx.not_null = '1' THEN ' NOT NULL' END;
      vFirst := vFirst + 1;
    END LOOP;
    vBuff := vBuff||') '||CASE WHEN inTablespaceName IS NOT NULL THEN 'tablespace '||inTablespaceName END||'}'';';
    ...
    FOR idx IN (
      SELECT REPLACE(REPLACE(trg_plsql,UPPER(GetVarValue('vOwner'))||'.',UPPER(vOwner)||'.'),LOWER(GetVarValue('vOwner'))||'.',LOWER(vOwner)||'.') AS trg_plsql
        FROM tb_trg_registry trg
             INNER JOIN tb_tables_registry t
               ON t.id = trg.table_id
                  AND t.table_name = UPPER(inTableName)
    ) LOOP
      vBuff := idx.trg_plsql||CHR(10);
      dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
    END LOOP;
  END IF;
    ...
    FOR idx IN (
      SELECT t.table_name
        ...
        IF idx.pk_key_name IS NULL THEN
          vBuff := vBuff||CHR(10)||CASE WHEN idx.key_type LIKE '%INDEX' THEN 'EXECUTE IMMEDIATE q''{CREATE '||idx.key_type||' '||idx.key_name||' ON '||idx.table_name||' ('||idx.key_fields||') '||CASE WHEN inTablespaceName IS NOT NULL THEN 'tablespace '||inTablespaceName END||' STORAGE(INITIAL 64K NEXT 1M)}'';'
                                   ELSE 'EXECUTE IMMEDIATE q''{ALTER TABLE '||idx.table_name||' ADD CONSTRAINT '||idx.key_name||' '||idx.key_type||' ('||idx.key_fields||')'||CASE WHEN idx.key_type = 'PRIMARY KEY' THEN ' USING INDEX TABLESPACE '||inTablespaceName||'}'';' WHEN idx.key_type = 'FOREIGN KEY' THEN ' REFERENCES '||idx.fk_table||' ('||idx.r_fields||')'||CASE WHEN idx.fk_ondelete = 'CASCADE' THEN ' ON DELETE '||idx.fk_ondelete END||'}'';' ELSE '}'';' END END;
        END IF;
    END LOOP;
    vBuff := vBuff||CHR(10)||'END;'||CHR(10)||'/'||CHR(10);
    dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
...
FUNCTION RegistryGetDDLScript(inOwner VARCHAR2,inTablespaceName VARCHAR2, inExtFilter VARCHAR2 DEFAULT NULL) RETURN CLOB
  IS
    vOldOwner VARCHAR2(256) := GetVarValue('vOwner');
    vCLOB CLOB;
    vSQL VARCHAR2(32700);
    vBuff CLOB;
    vCur SYS_REFCURSOR;
    vTableName VARCHAR2(256);
    vTableComment VARCHAR2(4000);
    ---
    errCreateSeq EXCEPTION;
    errCreateTbl EXCEPTION;
    errCreateTrg EXCEPTION;
    errCreatePK EXCEPTION;
    errCreateFK EXCEPTION;
BEGIN
  dbms_lob.createtemporary(vCLOB,FALSE);
  ...
  BEGIN
    OPEN vCur FOR vSQL;
    LOOP
      FETCH vCur INTO vTableName,vTableComment;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := REPLACE(REPLACE(RegistryCreateObjDDL(vTableName,vTableComment,'10000',inOwner,inTablespaceName),UPPER(vOldOwner)||'.',UPPER(inOwner)||'.'),LOWER(vOldOwner)||'.',LOWER(inOwner)||'.');
      dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
    END LOOP;
    CLOSE vCur;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    LOOP
      FETCH vCur INTO vTableName,vTableComment;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := REPLACE(REPLACE(RegistryCreateObjDDL(vTableName,vTableComment,'01000',inOwner,inTablespaceName),UPPER(vOldOwner)||'.',UPPER(inOwner)||'.'),LOWER(vOldOwner)||'.',LOWER(inOwner)||'.');
      dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
    END LOOP;
    CLOSE vCur;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    LOOP
      FETCH vCur INTO vTableName,vTableComment;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := REPLACE(REPLACE(RegistryCreateObjDDL(vTableName,vTableComment,'00100',inOwner,inTablespaceName),UPPER(vOldOwner)||'.',UPPER(inOwner)||'.'),LOWER(vOldOwner)||'.',LOWER(inOwner)||'.');
      dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
    END LOOP;
    CLOSE vCur;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    LOOP
      FETCH vCur INTO vTableName,vTableComment;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := REPLACE(REPLACE(RegistryCreateObjDDL(vTableName,vTableComment,'00010',inOwner,inTablespaceName),UPPER(vOldOwner)||'.',UPPER(inOwner)||'.'),LOWER(vOldOwner)||'.',LOWER(inOwner)||'.');
      dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
    END LOOP;
    CLOSE vCur;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    LOOP
      FETCH vCur INTO vTableName,vTableComment;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := REPLACE(REPLACE(RegistryCreateObjDDL(vTableName,vTableComment,'00001',inOwner,inTablespaceName),UPPER(vOldOwner)||'.',UPPER(inOwner)||'.'),LOWER(vOldOwner)||'.',LOWER(inOwner)||'.');
      dbms_lob.writeappend(vCLOB,LENGTH(vBuff),vBuff);
    END LOOP;
    CLOSE vCur;
  EXCEPTION WHEN OTHERS THEN
...
FUNCTION RegistryGetDMLScript(inOwner IN VARCHAR2,inExtFilter IN VARCHAR2 DEFAULT NULL) RETURN CLOB
  IS
  vOwner VARCHAR2(30) := pkg_etl_signs.GetVarValue('vOwner');
  vSQL CLOB;
  vName_1 VARCHAR2(256);
  vName_2 VARCHAR2(256);
  vName_3 VARCHAR2(256);
  vMetaSQL CLOB;
  vColNames VARCHAR2(32700);
  vColValues VARCHAR2(32700);
  
  vOut CLOB;
  vBuff VARCHAR2(32700);
  vCur SYS_REFCURSOR;
  vCurFld SYS_REFCURSOR;
  vKey NUMBER;
  vKeyName VARCHAR2(30);
  vKeyVal VARCHAR2(32700);
  vPrevRowNum NUMBER := 0;
  vCou INTEGER := 0;
  --
  errSqlIsNull EXCEPTION;
BEGIN
  ...
  vSQL := q'{SELECT trg_name FROM (SELECT t.soft_type,t.table_name,REPLACE(REPLACE(trg.trg_name,'}'||UPPER(vOwner)||q'{.','}'||UPPER(inOwner)||q'{.'),'}'||LOWER(vOwner)||q'{.','}'||LOWER(inOwner)||q'{.') AS trg_name
      FROM tb_trg_registry trg
           INNER JOIN tb_tables_registry t
             ON t.id = trg.table_id)}';
  IF NOT(inExtFilter IS NULL) THEN vSQL := vSQL||CHR(10)||'WHERE '||inExtFilter; END IF;
  
  OPEN vCur FOR vSQL;
  IF NOT(vCur IS NULL) THEN
    LOOP
      FETCH vCur INTO vName_1;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := 'ALTER TRIGGER '||vName_1||' DISABLE;'||CHR(10);
      dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
    END LOOP;
  END IF;
  CLOSE vCur;
  ...
  vSQL := q'{SELECT key_name,table_name FROM (SELECT t.soft_type,tk.key_name,REPLACE(REPLACE(t.table_name,'}'||UPPER(vOwner)||q'{.','}'||UPPER(inOwner)||q'{.'),'}'||LOWER(vOwner)||q'{.','}'||LOWER(inOwner)||q'{.') AS table_name
      FROM tb_tablekey_registry tk
           INNER JOIN tb_tables_registry t
             ON t.id = tk.table_id WHERE tk.key_type = 'FOREIGN KEY')}';
  IF NOT(inExtFilter IS NULL) THEN vSQL := vSQL||CHR(10)||'WHERE '||inExtFilter; END IF;

  OPEN vCur FOR vSQL;
  IF NOT(vCur IS NULL) THEN
    LOOP
      FETCH vCur INTO vName_1,vName_2;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := 'ALTER TABLE '||vName_2||' DISABLE CONSTRAINT '||vName_1||';'||CHR(10);
      dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
    END LOOP;
  END IF;
  CLOSE vCur;
  ...
  vSQL := q'{SELECT table_name,table_name_old FROM (SELECT t.soft_type,REPLACE(REPLACE(t.table_name,'}'||UPPER(vOwner)||q'{.','}'||UPPER(inOwner)||q'{.'),'}'||LOWER(vOwner)||q'{.','}'||LOWER(inOwner)||q'{.') AS table_name
          ,t.table_name AS table_name_old
      FROM tb_tables_registry t)}';
  IF NOT(inExtFilter IS NULL) THEN vSQL := vSQL||CHR(10)||'WHERE '||inExtFilter; END IF;
  --dbms_output.put_line(vSQL);
  ...
  IF NOT(vCur IS NULL) THEN
    LOOP
      ...
      SELECT LISTAGG(col_name,',') WITHIN GROUP (ORDER BY col_num) AS ColNames
            ,LISTAGG(CASE col_type
                       WHEN 'VARCHAR2' THEN col_name
                       WHEN 'CLOB' THEN 'dbms_lob.substr('||col_name||',4000,1) AS '||col_name
                       WHEN 'DATE' THEN 'to_char('||col_name||',''DD.MM.RRRR HH24:MI:SS'') AS '||col_name
                       WHEN 'NUMBER' THEN 'to_char('||col_name||') AS '||col_name
                     END,',') WITHIN GROUP (ORDER BY col_num) AS ColValues
        INTO vColNames,vColValues
        FROM TABLE(pkg_etl_signs.DescribeColumns('SELECT * FROM '||vName_2)) WHERE col_type != 'BLOB';
      
      vMetaSQL := 'SELECT * FROM (SELECT row_number() OVER (ORDER BY null) AS rownum_key,'||vColValues||' FROM ('||vName_2||')) UNPIVOT INCLUDE NULLS (key_val FOR key_name IN ('||vColNames||'))';
      --
      vCou := 0;
      vPrevRowNum := 0;
      vBuff :=
      'BEGIN'||Chr(10);
      dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
      OPEN vCurFld FOR vMetaSQL;
      IF (vCurFld IS NOT NULL) THEN
        LOOP
          FETCH vCurFld INTO vKey,vKeyName,vKeyVal;
          EXIT WHEN vCurFld%NOTFOUND;
          vKeyVal := CASE WHEN vCou > 0 THEN ',' END||'q''{'||REPLACE(REPLACE(vKeyVal,UPPER(vOwner)||'.',UPPER(inOwner)||'.'),LOWER(vOwner)||'.',LOWER(inOwner)||'.')||'}''';
          IF vPrevRowNum < vKey AND vCou > 0 THEN 
             vKeyVal := ');'||CHR(10)||'/*'||vName_2||'*/ INSERT INTO '||vName_1||' ('||vColNames||') VALUES('||SUBSTR(vKeyVal,2);
          ELSIF vPrevRowNum < vKey THEN
            vKeyVal := '/*'||vName_2||'*/ INSERT INTO '||vName_1||' ('||vColNames||') VALUES('||vKeyVal;
          END IF;
          vBuff := vKeyVal;
          dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
          vPrevRowNum := vKey;
  ...
  IF NOT(vCur IS NULL) THEN
    LOOP
      FETCH vCur INTO vName_1;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := 'ALTER TRIGGER '||vName_1||' ENABLE;'||CHR(10);
      dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
    END LOOP;
  END IF;
  CLOSE vCur;
  ...
  IF NOT(vCur IS NULL) THEN
    LOOP
      FETCH vCur INTO vName_1,vName_2;
      EXIT WHEN vCur%NOTFOUND;
      vBuff := 'ALTER TABLE '||vName_2||' ENABLE CONSTRAINT '||vName_1||';'||CHR(10);
      dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
    END LOOP;
  END IF;
  CLOSE vCur;
  ...
  vSQL := q'{SELECT field_name,table_name,seq_name FROM (SELECT t.soft_type,f.field_name
          ,REPLACE(REPLACE(t.table_name,'}'||UPPER(vOwner)||q'{.','}'||UPPER(inOwner)||q'{.'),'}'||LOWER(vOwner)||q'{.','}'||LOWER(inOwner)||q'{.') AS table_name
          ,REPLACE(REPLACE(s.seq_name,'}'||UPPER(vOwner)||q'{.','}'||UPPER(inOwner)||q'{.'),'}'||LOWER(vOwner)||q'{.','}'||LOWER(inOwner)||q'{.') AS seq_name
      FROM tb_tfield_registry f
           INNER JOIN tb_seq_registry s
             ON SUBSTR(s.seq_name,INSTR(s.seq_name,'.') + 1) = f.sequence_name
           INNER JOIN tb_tablekey_registry tk
             ON tk.table_id = f.table_id
                AND tk.key_fields = f.field_name
                AND tk.key_type = 'PRIMARY KEY'
           INNER JOIN tb_tables_registry t
             ON t.id = f.table_id)}';
  IF NOT(inExtFilter IS NULL) THEN vSQL := vSQL||CHR(10)||'WHERE '||inExtFilter; END IF;
  ...
  IF NOT(vCur IS NULL) THEN
    LOOP
      FETCH vCur INTO vName_1,vName_2,vName_3;
      EXIT WHEN vCur%NOTFOUND;
      vBuff :=
      'DECLARE'||CHR(10)||
      '  vDecr NUMBER;'||CHR(10)||
      '  vMaxID NUMBER;'||CHR(10)||
      '  vNextval INTEGER;'||CHR(10)||
      'BEGIN'||CHR(10)||
      '  SELECT MAX('||vName_1||') INTO vMaxID FROM '||vName_2||';'||CHR(10)||
      '  vDecr := vMaxID - '||vName_3||'.nextval;'||CHR(10)||
      '  IF vDecr != 0 THEN EXECUTE IMMEDIATE ''ALTER SEQUENCE '||vName_3||' INCREMENT BY ''||vDecr||'' NOCACHE''; END IF;'||CHR(10)||
      '  vNextVal := '||vName_3||'.nextval;'||CHR(10)||
      '  EXECUTE IMMEDIATE ''ALTER SEQUENCE '||vName_3||' INCREMENT BY 1 NOCACHE'';'||CHR(10)||
      'END;'||CHR(10)||'/'||CHR(10);
      dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
    END LOOP;
  END IF;
  ...
  vBuff :=
  'BEGIN'||CHR(10)||
  '  UPDATE tb_variable_registry SET val = '''||UPPER(inOwner)||''' WHERE NAME = ''vOwner'';'||CHR(10)||
  '  COMMIT;'||CHR(10)||
  'END;'||CHR(10)||'/'||CHR(10);
  dbms_lob.writeappend(vOut,LENGTH(vBuff),vBuff);
  RETURN vOut;
EXCEPTION WHEN OTHERS THEN
  pr_log_write(vOwner||'.pkg_etl_signs.RegistryGetDMLScript','ERROR OTHERS :: '||vBuff||' :: '||SQLERRM);
  RETURN NULL;
END RegistryGetDMLScript;  
