...
begin
  if TSelectEdt(Owner).DataType = 'String' then TSelectEdt(Owner).KeyVal := vQry.Fields[1].AsString;
  if TSelectEdt(Owner).DataType = 'Double' then TSelectEdt(Owner).KeyVal := vQry.Fields[1].AsFloat;
  if TSelectEdt(Owner).DataType = 'DateTime' then TSelectEdt(Owner).KeyVal := vQry.Fields[1].AsDateTime;
  if TSelectEdt(Owner).DataType = 'Boolean' then TSelectEdt(Owner).KeyVal := vQry.Fields[1].AsInteger;
